...
function is_restricted_file($file_to_view, $user_info, $developer_level) {
    ...
    $users_data = json_decode(file_get_contents($users_file), true);
    $user_info = $users_data[$token];
    // Check if 'allowed_dirs' key exists in the $user_info array
    if (!isset($user_info['allowed_dirs'])) {
        // Handle the case where 'allowed_dirs' is not defined
        $allowed_dirs = '';

    } else {
        $allowed_dirs = $user_info['allowed_dirs'];
    }

    // Replace URL encoded slashes with normal slashes
    $file_to_view = str_replace('%2F', '/', $file_to_view);
    ...
    // Check if the allowed directory is empty
    if (empty($allowed_dirs)) {
        // If the user does not have an allowed dir, deny access to all
        echo "Access denied: No allowed directory for the user.\n";
        return true;
    }

    // Check if the file is in the allowed directory

    if (strpos($file_to_view, $allowed_dirs) === 0) { // Ensure the file starts with the allowed dir

        return false; // File is in the allowed directory, allow access
    }

    // If the file is not in the allowed directory, restrict access
    echo "Access denied: File is not in the allowed directory.\n";
    return true;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'newfolder':
    check_permission(4);
    if (is_restricted_file($_POST['newfoldername'], $restricted_files, $developer_level)) {
                    die("Access denied to 'make new folder' restricted file.");
                }
    ...
    break;
            ...
            case 'move':
                check_permission(4);
                if (is_restricted_file($_POST['newpath'], $restricted_files, $developer_level)) {
            ...
            case 'newfile':
                ...
                if (is_restricted_file($_POST['newfilename'], $restricted_files, $developer_level)) {
                    die("Access denied to make new file in restricted area.");
                }
            ...
            case 'upload':
                ...
                if (isset($_FILES['file'])) {
                    ...
                    if (is_restricted_file($target_file, $restricted_files, $developer_level)) {
                    die("Access denied to make new file in restricted area.");
                }
            ...
            case 'compress':
                check_permission(4);
                if (is_restricted_file($_POST['filename'], $restricted_files, $developer_level)) {
                    die("Access denied to compress file in restricted area.");
                }
                try {
                        if (strpos(realpath($_POST['filename']), realpath($current_dir . '/Users')) === 0) {
                            check_permission(5);
...
if (isset($_GET['download'])) {
    check_permission(3);
    if (is_restricted_file($_POST['download'], $restricted_files, $developer_level)) {
                    die("Access denied to download restricted file.");
                }
...
if (isset($_GET['view'])) {
    if (is_restricted_file($_GET['view'], $restricted_files, $developer_level)) {
                    die("Access denied to view restricted file.");
                }
